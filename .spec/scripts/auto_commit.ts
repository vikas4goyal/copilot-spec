...
const { info: logInfo, error: logErrorMsg, log: logMsg } = createLogger("auto-commit");
...
if (!commitMessage) {
  logErrorMsg("Missing required --commit-message");
  process.exit(1);
}
...
if (runCommand(["git", "--version"], repoRoot).code !== 0) {
  ...
  logErrorMsg("Git not found; skipping commit.");
  process.exit(0);
}
...
if (runCommand(["git", "rev-parse", "--is-inside-work-tree"], repoRoot).code !== 0) {
  ...
  logErrorMsg("Not a Git repository; skipping commit.");
  process.exit(0);
}
...
if (gitAddResult.code !== 0) {
  logErrorMsg(`git add failed: ${gitAddResult.out}`);
  process.exit(1);
}
...
if (gitCommitResult.code !== 0) {
  logErrorMsg(`git commit failed: ${gitCommitResult.out}`);
  process.exit(1);
}

logInfo("Commit completed successfully");
logMsg("Committed successfully.");
