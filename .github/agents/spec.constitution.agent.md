...
2. Load the existing constitution at `.spec/memory/constitution.md`.
...
8. Write the completed constitution back to `.spec/memory/constitution.md` (overwrite).
...
1. Run the following command:
   ...
   **Note:** `--handoff-agent spec.specify` is a suggestion. `post_agent.ts` will ignore it if `specify` is already complete (prevents rework loops). Only takes effect when `specify` status is `pending`.
